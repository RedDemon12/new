...
.bx.bx-compass.blink.color1 {
  color: red;
}

.bx.bx-compass.blink.color2 {
  color: blue;
}

.bx.bx-compass.blink.color3 {
  color: green;
}
...
<?php
$query012 = mysqli_query($con, "SELECT * FROM message WHERE mssg_to = {$_SESSION['id']} AND status='unread'");
$rowstar = mysqli_fetch_array($query012);

if ($count == '0') {
  
  echo '<i class="bx bx-compass" aria-hidden="true" style="color: black; font-size: 32px; margin-top: 1px; margin-right: 7px;"></i>';
} else {
  
  echo '<i class="bx bx-compass blink" aria-hidden="true" style="font-size: 32px; margin-top: 1px; margin-right: 7px;" ></i>';
}
?></a>
<script>
  
  const icon = document.querySelector('.blink');
...
?>
                            ...
                            <input
                              ...
                              <button type="submit" name="submit "id="post-textbtn" style="display:none">Post</button>

 
<script>
var inputField = document.getElementById("text-link");
var postButton = document.getElementById("post-textbtn");
...
document.addEventListener("click", function(event) {
  if (event.target !== inputField) {
    postButton.style.display = "none";
                          ...
                          <li><a href="#" id="video-link">Video</a></li>
                          <li><a href="#" id="audio-link">Audio</a></li>
                          <li><a href="#" id="doc-link">Document</a></li>
...
<input name="profile-video" id="video-input" type="file" accept=".mp4,.wav" style="display: none;">
<input name="profile-audio" id="audio-input" type="file" accept=".mp3" style="display: none;">
<input name="profile-doc" id="doc-input" type="file" accept=".pdf" style="display: none;">
...
<div id="modal" class="modal">
  <div class="modal-content">
    ...
    <div class="styled-select" id="hello" style="display:none;">
...
<img id="selected-image" style="display:none;" src="">
<video style="width: 25%; height: 25%; display:none;" id="selected-video" src=""></video>
<audio style="width: 25%; height: 25%; display:none;" id="selected-audio" src=""></audio>
<embed style="width: 25%; height: 25%; display:none;" id="selected-document" src="" type="application/pdf" />
...
<div id="tag-selections"></div>
  ...
  <ul id="tag-suggestions"></ul>
   ...
   <b> <p id="location" style="display:none;"></p></b>
    ...
    <button type="submit" name="submit "id="post-btn" style="display:none">Post</button>
    <button type="submit" name="submit "id="post-vidbtn" style="display:none">Post</button>
    <button type="submit" name="submit "id="post-audbtn" style="display:none">Post</button>
    <button type="submit" name="submit "id="post-docbtn" style="display:none">Post</button>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    var postVidBtn = document.getElementById("post-vidbtn");
    var postAudBtn = document.getElementById("post-audbtn");
    var postDocBtn = document.getElementById("post-docbtn");
    ...
    // Add click event listener to the post video button
    postVidBtn.addEventListener("click", function() {
      ...
      // Send an AJAX request to insert the video post into the database
      var xhr = new XMLHttpRequest();
      xhr.open("POST", "vidposting.php");
      xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
          //  window.location.reload();
      ...
      xhr.send("location=" + location.textContent + "&caption=" + caption.value + "&userId=" + userId  + "&fontSize=" + selectedFontSize);
    });

    postAudBtn.addEventListener("click", function() {
    ...
    postDocBtn.addEventListener("click", function() {
...
document.getElementById("doc-link").addEventListener("click", function(event) {
  event.preventDefault();
...
document.getElementById("file-input").addEventListener("change", function() {
  ...
  document.getElementById("selected-audio").style.display = "none";
...
document.getElementById("video-input").addEventListener("change", function() {
  ...
  document.getElementById("selected-audio").style.display = "none";
  document.getElementById("post-vidbtn").style.display = "block";
  document.getElementById("modal").style.display = "block";
  video.play();
});

document.getElementById("audio-input").addEventListener("change", function() {
  var audio = document.getElementById("selected-audio");
  ...
  document.getElementById("selected-image").style.display = "none";
  document.getElementById("selected-video").style.display = "none";
  document.getElementById("post-audbtn").style.display = "block";
  document.getElementById("modal").style.display = "block";
  audio.play();
});

document.getElementById("doc-input").addEventListener("change", function() {
  var documentPreview = document.getElementById("selected-document");
  documentPreview.src = URL.createObjectURL(this.files[0]);
  documentPreview.style.display = "inline-block";
  documentPreview.style.width = "30%";
  documentPreview.style.height = "auto";
  documentPreview.style.marginBottom = "10px";
  document.getElementById("selected-image").style.display = "none";
  document.getElementById("selected-video").style.display = "none";
  document.getElementById("post-docbtn").style.display = "block";
  document.getElementById("modal").style.display = "block";
});
...
<?php 
$query0 = mysqli_query($con, "SELECT follow_to FROM follow_system WHERE follow_by = {$_SESSION['id']}");
$postIds = array();

while($rows = mysqli_fetch_array($query0)){
  $query1 = mysqli_query($con, "SELECT post_id FROM post WHERE user_id = '".$rows['follow_to']."' OR user_id = '".$_SESSION['id']."' ORDER BY time DESC");
  while($rowss = mysqli_fetch_array($query1)) {
        $postIds[] = $rowss['post_id'];
    }
}
...
&nbsp;
    ...
    <h5> <?php echo '<a href="clickablepro.php?id=' . $row['id'] . '">'?><?php echo htmlentities($row['username']); ?></a><br> <span style="  font-size: small;"><?php echo htmlentities($row['firstname'] . ' ' . $row['surname']); ?><span></h5><?php }?>    
...
$rows = mysqli_fetch_array($query); ?>
  ...
  <div class="dropdowns-content">
    <?php
  if($rows['user_id'] == $postId || $rows['user_id'] == $_SESSION['id']) {
    echo '<a href="#" onclick="openEditForm(' . $postId . ')">Edit Post</a>'; 
    echo '<a href="#" onclick="openDeleteForm(' . $postId . ')">Delete Post</a>';}?>
<?php
echo '<a href="#" onclick="reportPost(' . $rows['user_id'] . ', ' . $postId . ')">Report Post</a>';
?>
...
function openEditForm(postId) {
  console.log("openEditForm called with postId=", postId);
  ...
  xhr.open('GET', 'edit_post.php?post_id=' + postId, true);
  xhr.send();
...
function openDeleteForm(postId) {
  ...
  xhr.open('GET', 'del_post.php?post_id=' + postId, true);
  xhr.send();
...
<script>
function reportPost(user_id, post_id) {
  console.log('User ID:', user_id);
  console.log('Post ID:', post_id);

  $.ajax({
    type: 'POST',
    url: 'adminhomepanel.php',
    data: {
      'user_id': user_id,
      'post_id': post_id
    },
...
$row = mysqli_fetch_array($query); ?>
<span class="lik" style= "cursor: pointer" id="likes_<?php echo $postId; ?>" onclick="showLikesModal('<?php echo $postId; ?>')"> <?php echo htmlentities($row['likes']); ?> Likes</span>

<?php  
$query = mysqli_query($con, "SELECT COUNT(*) as comments FROM post_comments WHERE post_id = '$postId'");  
$row = mysqli_fetch_array($query); ?>
<span class="com"  style= "cursor: pointer" id="comments_<?php echo $postId; ?>" onclick="showCommentsModal('<?php echo $postId; ?>')"> <?php echo htmlentities($row['comments']); ?> Comments</span>
...
<div class="post" id="post_<?php echo $postId ?>">
  <?php
$userId = $_SESSION['id'];
  $query = mysqli_query($con, "SELECT * FROM post_likes WHERE post_id = '$postId' AND post_like_by = '$userId'");
  $isLiked = mysqli_num_rows($query) > 0;
  ?>
  <button class="like-button<?php echo $isLiked ? ' liked' : ''; ?>" onclick="toggleLike(this)" data-post-id="<?php echo $postId ?>">
...
function toggleLike(button) {
  ...
  xhr.onload = function() {
    if (xhr.status === 200 && xhr.responseText) {
      ...
      var likeCountElem = document.querySelector('#likes_' + postId);
  ...
  <div class="comment-input" style="display:none;">
    ...
    <button class="send-button" data-post-id="<?php echo $postId; ?>">
...
$sql = "SELECT * FROM bookmarks WHERE post_id='$postId' AND user_id='$userId'";
$result = mysqli_query($con, $sql);
...
<button class="<?php echo $buttonClass ?>" onclick="toggleBookmark(this)" data-post-id="<?php echo $postId ?>" data-user-id="<?php echo $userId ?>">
...
const postButton1 = document.getElementById('post-btn');

  postButton1.addEventListener('click', function(event) {
    event.preventDefault(); 
...
const postvidButton = document.getElementById('post-vidbtn');


  postvidButton.addEventListener('click', function(event) {
    event.preventDefault(); 

    const files = vidInput.files[0];
...
const postaudButton = document.getElementById('post-audbtn');


  postaudButton.addEventListener('click', function(event) {
    event.preventDefault(); 

    const files = audInput.files[0];
...
const postdocButton = document.getElementById('post-docbtn');


  postdocButton.addEventListener('click', function(event) {
    event.preventDefault(); 

    const files = docInput.files[0];
    ...
    $.ajax({
      ...
      contentType: false,
      success: function(response) {
        console.log('Doc uploaded successfully:', response);
window.location.reload();
      },
      error: function(error) {
        console.error('Error uploading video:', error);
...
<script>
const textInput = document.getElementById('text-link');
const posttextButton = document.getElementById('post-textbtn');
var userId = "<?php echo $_SESSION['id']; ?>";

posttextButton.addEventListener('click', function(event) {
  event.preventDefault(); 
  var xhr = new XMLHttpRequest();
      xhr.open("POST", "txtposting.php");
      xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
          //  window.location.reload();
