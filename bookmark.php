                      ...
                      <div class="card">
                        <div class="card-body">
                          <div class="card-title d-flex align-items-start justify-content-between">
                           ...
                           <span style="color:black; font-size: larger; text-align: center;">SAVED POSTS</span>
...
$query = mysqli_query($con, "SELECT post_id FROM bookmarks WHERE user_id = {$_SESSION['id']}");
$book = array();
while($row = mysqli_fetch_array($query)) {
    $book[] = $row['post_id'];
}
foreach($book as $books) {
  $image_sql = "SELECT post_id, image, about FROM image_post WHERE post_id = $books";
  $image_result = mysqli_query($con, $image_sql);

  $video_sql = "SELECT post_id, video, about FROM video_post WHERE post_id = $books";
  $video_result = mysqli_query($con, $video_sql);

  $audio_sql = "SELECT post_id, audio, about FROM audio_post WHERE post_id = $books";
  $audio_result = mysqli_query($con, $audio_sql);

  $doc_sql = "SELECT post_id, doc, about FROM doc_post WHERE post_id = $books";
  $doc_result = mysqli_query($con, $doc_sql);

  $txt_sql = "SELECT post_id, text FROM text_post WHERE post_id = $books";
  $txt_result = mysqli_query($con, $txt_sql);
  // Merge the results into a single array
  $result = array_merge(mysqli_fetch_all($image_result, MYSQLI_ASSOC), mysqli_fetch_all($video_result, MYSQLI_ASSOC),mysqli_fetch_all($audio_result, MYSQLI_ASSOC),mysqli_fetch_all($doc_result, MYSQLI_ASSOC),mysqli_fetch_all($txt_result, MYSQLI_ASSOC));
  ...
  foreach($result as $row) {
      if ($i % 3 == 0) {
          ...
          if ($i == 0) {
              ...
              echo "</div><div class='row' style='margin-bottom: 30px;'>";
          }
      }
      echo "<div class='col'>";
      if (isset($row["image"])) {
          ...
          echo "</a>";
      } else if (isset($row["video"])) {
          // Display video if image doesn't exist but video exists
          echo "<a href='post.php?id=" . $row["post_id"] . "&user_id=" . $user_id . "'>";

          echo "<video width='200' height='200' controls>";
          echo "<source src='videoposts/" . $row["video"] . "' type='video/mp4'>";
          echo "Your browser does not support the video tag.";
          echo "</video>";
          echo "</a>";

      } else if (isset($row["audio"])) {
        // Display audio if image and video don't exist but audio exists
        echo "<a href='post.php?id=" . $row["post_id"] . "&user_id=" . $user_id . "'>";
        echo "<audio controls>";
        echo "<source src='audioposts/" . $row["audio"] . "' type='audio/mpeg'>";
        echo "</audio>";
        echo "</a>";
      }
      else if (isset($row["doc"])) {
        // Display document if image, video and audio don't exist but document exists
        echo "<a href='post.php?id=" . $row["post_id"] . "&user_id=" . $user_id . "'>";
        echo '<iframe src="http://localhost/temp/docposts/' . $row['doc'] . '" frameborder="0" style="width:50%; height:300px;"></iframe>';
          echo "</a>";
      }
        ...
        echo '<p style="font-size: 1.2rem; line-height: 1.5; color: #333; font-family: Montserrat;">' . $row["text"] . '</p>';
        echo '</div>';
        echo "</a>";
      }
